...
void sin_function()
{
   ...
   /* float temp=result;
    ...
    for(i=1;i<acc;i++)
    {
        temp=(-1)*angle_radian*angle_radian*angle_radian/((i*2)*(i*2+1));
        result = result+temp;
    }

    printf("\n\t\tResult for sin(%d) is : %.3f",angle_degree,result);
    getch();


    double sum,x,term,n,y;
    int i;
    ...
    for(i=0;i<n;i++)
    {
        term=(-1)*term*x*x/((2*1)*(2*1+1));
        printf("\n\t\tThe sin(%lf) is %.4lf",y,sum);
    }
    */
    getch();
...
void cosine_function()
{
   long int angle_degree,i;
    ...
    printf("\n\t\tResult for cos(%ld) is : %.2f",angle_degree,result);*/
    getch();
}
